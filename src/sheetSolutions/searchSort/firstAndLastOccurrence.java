...
public class firstAndLastOccurrence {
  ...
  static int first(int[] ar, int low, int high, int x) {
    ...
    if (low <= high) {
      mid = low + (high - low) / 2;
      if ((mid == 0 || ar[mid - 1] < x) && ar[mid] == x) return mid;
      else if (x > ar[mid]) return first(ar, mid + 1, high, x);
      else return first(ar, low, mid - 1, x);
    }
    return -1;
  }
  ...
  static int last(int[] ar, int low, int high, int x) {
    if (low <= high) {
      ...
      if ((mid == ar.length - 1 || ar[mid + 1] > x) && ar[mid] == x) {
        return mid;
      } else if (x < ar[mid]) return last(ar, low, mid - 1, x);
      else return last(ar, mid + 1, high, x);
    }
    return -1;
  }
